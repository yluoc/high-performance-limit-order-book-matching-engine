...
TEST(limit_test, insert_multiple_orders) {
	OrderPointer order1 = std::make_shared<Order>(1, 1, BUY, 100, 50, 50, ACTIVE);
	OrderPointer order2 = std::make_shared<Order>(2, 1, BUY, 100, 30, 30, ACTIVE);
	OrderPointer order3 = std::make_shared<Order>(3, 1, BUY, 100, 20, 20, ACTIVE);
	...
	EXPECT_EQ(limit.get_order_number(), 3);
...
TEST(limit_test, delete_order_from_limit) {
	...
	EXPECT_EQ(limit.get_order_number(), 2);
	EXPECT_EQ(limit.get_total_volume(), 70);
}

TEST(limit_test, match_order_partial_fill) {
	OrderPointer buy_order = std::make_shared<Order>(1, 1, BUY, 100, 50, 50, ACTIVE);
	OrderPointer sell_order = std::make_shared<Order>(2, 2, SELL, 100, 30, 30, ACTIVE);
...
TEST(limit_test, match_order_full_fill) {
	OrderPointer buy_order = std::make_shared<Order>(1, 1, BUY, 100, 50, 50, ACTIVE);
	OrderPointer sell_order1 = std::make_shared<Order>(2, 2, SELL, 100, 30, 30, ACTIVE);
	OrderPointer sell_order2 = std::make_shared<Order>(3, 3, SELL, 100, 20, 20, ACTIVE);
	...
	EXPECT_EQ(buy_order->get_remaining_volume(), 0);
	EXPECT_EQ(sell_order1->get_remaining_volume(), 0);
	EXPECT_EQ(sell_order2->get_remaining_volume(), 0);
	EXPECT_EQ(limit.get_order_number(), 0);
...
TEST(limit_test, match_order_with_remaining_volume) {
	OrderPointer buy_order = std::make_shared<Order>(1, 1, BUY, 100, 50, 50, ACTIVE);
	OrderPointer sell_order1 = std::make_shared<Order>(2, 2, SELL, 100, 30, 30, ACTIVE);
	OrderPointer sell_order2 = std::make_shared<Order>(3, 3, SELL, 100, 10, 10, ACTIVE);
	...
	EXPECT_EQ(buy_order->get_remaining_volume(), 10);
	EXPECT_EQ(sell_order1->get_remaining_volume(), 0);
	EXPECT_EQ(sell_order2->get_remaining_volume(), 0);
}
...
TEST(book_test, place_sell_order_no_match) {
	Book book;
	OrderPointer sell_order = std::make_shared<Order>(1, 1, SELL, 100, 50, 50, ACTIVE);
...
TEST(book_test, place_buy_order_with_match) {
	Book book;
	OrderPointer sell_order = std::make_shared<Order>(1, 1, SELL, 100, 30, 30, ACTIVE);
	book.place_order(sell_order);

	OrderPointer buy_order = std::make_shared<Order>(2, 2, BUY, 100, 50, 50, ACTIVE);
	Trades trades = book.place_order(buy_order);
...
TEST(book_test, place_sell_order_with_match) {
	Book book;
	OrderPointer buy_order = std::make_shared<Order>(1, 1, BUY, 100, 30, 30, ACTIVE);
	book.place_order(buy_order);

	OrderPointer sell_order = std::make_shared<Order>(2, 2, SELL, 100, 50, 50, ACTIVE);
	Trades trades = book.place_order(sell_order);

	EXPECT_EQ(trades.size(), 1);
	EXPECT_EQ(trades[0].get_trade_volume(), 30);
	EXPECT_EQ(sell_order->get_remaining_volume(), 20);
	EXPECT_EQ(buy_order->get_remaining_volume(), 0);
...
TEST(book_test, multiple_orders_same_price) {
	Book book;
	OrderPointer buy_order1 = std::make_shared<Order>(1, 1, BUY, 100, 30, 30, ACTIVE);
	OrderPointer buy_order2 = std::make_shared<Order>(2, 1, BUY, 100, 20, 20, ACTIVE);
	book.place_order(buy_order1);
	book.place_order(buy_order2);

	OrderPointer sell_order = std::make_shared<Order>(3, 2, SELL, 100, 40, 40, ACTIVE);
	Trades trades = book.place_order(sell_order);
	...
	EXPECT_EQ(trades[1].get_trade_volume(), 10);
	EXPECT_EQ(sell_order->get_remaining_volume(), 0);
	EXPECT_EQ(buy_order1->get_remaining_volume(), 0);
	EXPECT_EQ(buy_order2->get_remaining_volume(), 10);
...
TEST(book_test, place_order_with_different_prices) {
	Book book;
	OrderPointer buy_order1 = std::make_shared<Order>(1, 1, BUY, 100, 30, 30, ACTIVE);
	OrderPointer buy_order2 = std::make_shared<Order>(2, 1, BUY, 110, 20, 20, ACTIVE);
	book.place_order(buy_order1);
	book.place_order(buy_order2);

	OrderPointer sell_order = std::make_shared<Order>(3, 2, SELL, 105, 40, 40, ACTIVE);
	Trades trades = book.place_order(sell_order);

	EXPECT_EQ(trades.size(), 1);
	EXPECT_EQ(trades[0].get_trade_volume(), 20);
	EXPECT_EQ(sell_order->get_remaining_volume(), 20);
	EXPECT_EQ(sell_order->get_order_status(), ACTIVE);
	EXPECT_EQ(buy_order1->get_remaining_volume(), 30);
	EXPECT_EQ(buy_order1->get_order_status(), ACTIVE);
	EXPECT_EQ(buy_order2->get_remaining_volume(), 0);
	EXPECT_EQ(buy_order2->get_order_status(), FULFILLED);
...
TEST(book_test, place_order_with_invalid_price) {
	Book book;
	OrderPointer buy_order = std::make_shared<Order>(1, 1, BUY, 0, 30, 30, ACTIVE);
	Trades trades = book.place_order(buy_order);
...
TEST(book_test, match_orders_with_multiple_limits) {
	...
	OrderPointer buy_order1 = std::make_shared<Order>(1, 1, BUY, 100, 30, 30, ACTIVE);
	OrderPointer buy_order2 = std::make_shared<Order>(2, 1, BUY, 110, 20, 20, ACTIVE);
	OrderPointer sell_order1 = std::make_shared<Order>(3, 2, SELL, 105, 15, 15, ACTIVE);
	OrderPointer sell_order2 = std::make_shared<Order>(4, 2, SELL, 95, 30, 30, ACTIVE);
	book.place_order(buy_order1);
	book.place_order(buy_order2);
	...
	EXPECT_EQ(trades.size(), 2);
	EXPECT_EQ(trades[0].get_trade_volume(), 5);
	EXPECT_EQ(trades[1].get_trade_volume(), 25);
